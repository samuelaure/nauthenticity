...
import { ArrowLeft, ArrowRight, MessageCircle, Heart, Eye, Calendar, Edit, Save, X } from 'lucide-react';
...
export const PostView = () => {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: { caption: string; transcriptText: string }) =>
            updatePost(id!, data),
        onSuccess: () => {
            setIsEditing(false);
    ...
    return (
        <div className="fade-in" style={{ maxWidth: '1000px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <Link to={`/accounts/${post.username}`} style={{ color: 'var(--text-secondary)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <ArrowLeft size={16} /> Back to Posts
                </Link>
                <div style={{ display: 'flex', gap: '1rem' }}>
                    {post.newerPostId && (
                        <Link to={`/posts/${post.newerPostId}`} className="nav-arrow" title="Previous Post (Newer)" onClick={() => setIsEditing(false)}>
                            <ArrowLeft size={24} />
                        </Link>
                    )}
                    {post.olderPostId && (
                        <Link to={`/posts/${post.olderPostId}`} className="nav-arrow" title="Next Post (Older)" onClick={() => setIsEditing(false)}>
                            <ArrowRight size={24} />
                        </Link>
                    )}
                </div>
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                            {isEditing ? (
                                <>
                                    <button onClick={handleSave} className="action-btn" title="Save" disabled={updateMutation.isPending}>
                                        <Save size={18} />
                                    </button>
                                    <button onClick={() => setIsEditing(false)} className="action-btn" style={{ background: '#555' }} title="Cancel">
                                        <X size={18} />
                                    </button>
                                </>
                            ) : (
                                <button onClick={() => setIsEditing(true)} className="action-btn" title="Edit Content">
                                    <Edit size={18} />
                                </button>
                            )}
                        </div>
                    ...
                    <div style={{ marginBottom: '2rem' }}>
                        ...
                        {isEditing ? (
                            <textarea
                                ...
                                style={{ width: '100%', minHeight: '150px', background: '#000', color: '#fff', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border)' }}
                            />
                        ) : (
                            <div style={{ whiteSpace: 'pre-wrap', maxHeight: '200px', overflowY: 'auto', paddingRight: '0.5rem', color: '#ccc', lineHeight: '1.6' }} className="custom-scrollbar">
                                {post.caption}
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {isEditing ? (
                            <textarea
                                ...
                                style={{ width: '100%', minHeight: '300px', background: '#000', color: '#fff', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border)' }}
                            />
                        ) : (
