    ...
    return (
        <div className="fade-in">
            ...
            <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '1rem' }}>
                <select value={sort} onChange={(e) => setSort(e.target.value as any)} style={{ padding: '0.5rem', borderRadius: '4px', background: 'var(--card-bg)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}>
            ...
            <div className="posts-grid">
                {sortedPosts.map(post => (
                    <Link to={`/posts/${post.id}`} key={post.id} className="post-card" style={{ textDecoration: 'none' }}>
                        {post.media && post.media.length > 0 ? (
                            post.media[0].type === 'video' ? (
                                <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                                    ...
                                    />
                                    <div style={{ position: 'absolute', top: 5, right: 5, background: 'rgba(0,0,0,0.6)', borderRadius: '4px', padding: '2px 4px' }}>
                                        🎥
                                    </div>
                                </div>
                            ) : (
                            ...
                            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#222', color: '#666' }}>
                                No Media
                            </div>
                        )}
