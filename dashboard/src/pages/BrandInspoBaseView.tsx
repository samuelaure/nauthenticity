  ...
  return (
    <div className="fade-in">
      ...
      <section style={{ marginBottom: '3rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: '#c9d1d9' }}>
          ...
          <div style={{ 
            ...
            {digest.attachedUrls && digest.attachedUrls.length > 0 && (
              <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
                <span style={{ fontSize: '0.8rem', color: '#8b949e', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Top Influences
                </span>
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#c9d1d9' }}>
            <Calendar size={18} />
            <h2 style={{ fontSize: '1.25rem', margin: 0, border: 'none' }}>Captured Inspo ({items?.length || 0})</h2>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
            ...
            <div key={item.id} style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <span style={{ 
                  ...
                  padding: '2px 8px', 
                  borderRadius: '100px', 
                  background: item.type === 'inspo' ? 'rgba(56, 139, 253, 0.15)' : 'rgba(163, 113, 247, 0.15)',
              ...
              {item.post && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(0,0,0,0.2)', padding: '8px', borderRadius: '6px' }}>
                  <div style={{ width: '40px', height: '40px', background: '#333', borderRadius: '4px', overflow: 'hidden', flexShrink: 0 }}>
                  ...
                  <div style={{ overflow: 'hidden' }}>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem', color: '#f0f6fc' }}>@{item.post.username}</div>
                    <a href={item.post.instagramUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.75rem', color: '#8b949e', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '2px' }}>
              ...
              {item.note && (
                <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                  <MessageSquare size={14} style={{ color: '#8b949e', marginTop: '4px', flexShrink: 0 }} />
                  <p style={{ margin: 0, fontSize: '0.9rem', color: '#c9d1d9', fontStyle: 'italic' }}>
                    "{item.note}"
                  </p>
                </div>
              )}
              ...
              <div style={{ marginTop: 'auto', paddingTop: '0.5rem', display: 'flex', justifyContent: 'flex-end' }}>
                 <span style={{ 
                   ...
                   <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: item.status === 'processed' ? '#3fb950' : '#d29922' }} />
          ...
          {items?.length === 0 && (
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem', color: '#8b949e' }}>
